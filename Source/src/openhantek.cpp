...
void OpenHantekMainWindow::createActions() {
    ...
    connect(this->bufferSizeActionGroup, SIGNAL(triggered(QAction *)), this, SLOT(bufferSizeTriggered(QAction *)));
    ...
    this->bufferSizeSmallAction->setChecked(this->settings->scope.horizontal.samples == Buudai::BUFFER_SMALL);
    this->bufferSizeSmallAction->setStatusTip(tr("%1 Samples").arg(Buudai::BUFFER_SMALL));
    ...
    this->bufferSizeLargeAction->setChecked(this->settings->scope.horizontal.samples == Buudai::BUFFER_LARGE);
    this->bufferSizeLargeAction->setStatusTip(tr("%1 Samples").arg(Buudai::BUFFER_LARGE));
...
void OpenHantekMainWindow::createMenus() {
    ...
    // Selection Menu disabled due to buffersize selection is now done by samplerate calculation
...
void OpenHantekMainWindow::about() {
	QMessageBox::about(this, tr("About OpenHantek %1").arg(VERSION), tr(
        ...
        "<p>Copyright &copy; 2014, 2016 mmark</p>"
        "<p>Copyright &copy; 2015, 2016 Christian Weidner &lt;dr.mord@gmx.de&gt;</p>"
        "<p>Copyright &copy; 2016 Joe Vee &lt;krill2012@yahoo.com&gt;</p>"
        "<p>Additional support for SainSmart/Buudai DDS120.</p>"
        "<p>Check EEVBlog Forum.</p>"
	));
}
...
void OpenHantekMainWindow::bufferSizeTriggered(QAction *action) {

    // Do not mess with buffersize - FIX

//    this->settings->scope.horizontal.samples = (action == this->bufferSizeSmallAction) ? Buudai::BUFFER_SMALL : Buudai::BUFFER_LARGE;
//	this->dsoControl->setBufferSize(this->settings->scope.horizontal.samples);
}
